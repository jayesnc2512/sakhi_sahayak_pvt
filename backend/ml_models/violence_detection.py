...
class ViolenceDetector:
    ...
    def analyze_frame(frame, model, queue):
        ...
        processed_frame = ViolenceDetector.process_frame(frame)
        preds = model.predict(np.expand_dims(processed_frame, axis=0))[0]
        queue.append(preds)
        results = np.array(queue).mean(axis=0)
        return (results > 0.50)[0]  # Returns True if violence is detected
    ...
    @staticmethod
    async def run(video_path, websocket):
        """Run the violence detection on the video."""
        alert_duration=3
        model_path = "ml_models/vmodel.h5"
        model = ViolenceDetector.load_model(model_path)
        ...
        while True:
            ...
            is_violence = ViolenceDetector.analyze_frame(frame, model, queue)
            ViolenceDetector.display_output(frame, is_violence)
            ...
            if consecutive_violence_frames >= alert_duration * (video_capture.get(cv2.CAP_PROP_FPS) // 1):
                ...
                if current_time - last_alert_time >= alert_duration:
                    await websocket.send_json({"message":"Violence Detected"})
                    ViolenceDetector.log_alert()
                    consecutive_violence_frames, last_alert_time = ViolenceDetector.reset_alert()
